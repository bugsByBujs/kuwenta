...
export function PreAuthFlow({ initial = "onboarding" }: { initial?: Phase | "register" }) {
  ...
  async function doRegister() {
    ...
    // auto-confirm trigger lets us sign in immediately
  ...
  async function finishSetup() {
    ...
    try {
      ...
      const created = await q.createAccounts(
        chosen.map((b) => ({ name: b.label, type: b.type, balance: 0 }))
      );
      const byType = (t: string) => created.filter((a) => a.type === t);
      const banksA = byType("bank");
      const walletsA = byType("wallet");
      const cashA = byType("cash");
      ...
      const budget = {
        ...
        wantsPercent: 0.6,
        savingsPercent: 0.4,
        billsAccountId: banksA[0]?.id ?? first,
        dailySpendAccountId: cashA[0]?.id ?? first,
        miscAccountId: walletsA[0]?.id ?? first,
        efAccountId: banksA[1]?.id ?? banksA[0]?.id ?? first,
        extraAccountId: banksA[0]?.id ?? first,
      };
      await q.updateSettings({
        monthlySalary: salaryNum,
        workDays: pay.workDays,
        defaultAccountId: walletsA[0]?.id ?? first,
        budget,
        pay,
      });
      router.replace("/home");
